  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const data = await response.json();
        // 받아온 데이터로 values와 fields 상태를 업데이트합니다.
        setNickname(data.data.result.map(item => item[0])); // 숫자로 변환하여 저장
        setExplain(data.data.result.map(item => item[1])); // 텍스트로 저장
      } catch (error) {
        console.error('데이터 가져오기 실패:', error);
      }
    };
    ...
    <div className='mbti-container'>
      <div className='mbti-content'>
        <div className='mbti-text'>
          <span className='w-btn w-btn-blue' style={{ fontWeight: 'bold',fontSize:'30px' }}>당신의 쇼핑유형은 "{nickname}"입니다.</span>
          <br></br>
